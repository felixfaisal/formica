...
def start_form():
    ...
    return questions, q_count, responses
...
def set_response(responses, response, author, index):
...
def end_form(questions, responses, author_index):
    form_started = False
    confirmation_embed = discord.Embed(title = 'Confirm your answers', description = 'Click ✅ to submit.\n If you need to edit your answers, go back and do so, then type !finished.', color = form_color)
...
async def on_message(message):
    ...
    if msg.startswith('!start'):
        if form_started == False:
            cur_index = 0
            questions, q_count, responses = start_form()
        ...
        while cur_index < q_count:
            ...
            author_index = set_response(responses, msg.content, message.author, cur_index)
        ...
        confirmation_embed = end_form(questions, responses, author_index)
